local STRING_LISTS = {}
for k, v in pairs(STRINGS.VOIDSTEAD_QUOTES) do
...
local function OnEntityReplicated(inst)
    ...
    if inst._parent == nil then
        print("Unable to initialize classified data for Voidstead")
    else
...
local function fn()
    ...
    inst.string_list = net_smallbyte(inst.GUID, "voidstead_classified.string_list")
    inst.string_id = net_smallbyte(inst.GUID, "voidstead_classified.string_id")
    inst.sound_override = net_tinybyte(inst.GUID, "voidstead_classified.sound_override", "saydirty")
    inst.enabled = false
...
return Prefab("voidstead_classified", fn)
